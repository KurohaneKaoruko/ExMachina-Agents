...
阻断规则：在语言、全连结指挥体显示名、安装模式、目标配置路径与模型未确认前，不得导入 settings patch。
...
6. 主模型：主控体 / Lite 主入口使用哪个模型？
   模板变量：`{{OPENCLAW_PRIMARY_MODEL}}`

## 可选补充项
1. 快速模型：如果走 Full 模式，主控体快速模型是什么？
   模板变量：`{{OPENCLAW_FAST_MODEL}}`
   适用模式：full
2. 协作模型：如果走 Full 模式，协作 agent 使用哪个模型？
   模板变量：`{{OPENCLAW_SUPPORT_MODEL}}`
   适用模式：full
3. 宿主多 agent 能力：宿主是否支持多 agent 绑定与外部路由？
   默认值：False
4. 其它配置：还有哪些渠道绑定、token、workspace 或风格配置需要一并记录？
