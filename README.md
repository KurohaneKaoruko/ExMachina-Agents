...
| 层级 | 编号区间 | 数量 | 清单 |
|------|----------|-----:|------|
| 顶层指挥体 | `00_` | 1 | `00_全连结指挥体`（吸收原协调职能：任务分解、进度与资源管理） |
| 工作域连结体 | `01_` ~ `05_` | 5 | `01_研究与理性`、`02_架构与实作`、`03_校验与安全`、`04_集成与运维`、`05_文档与体验` |
| 子个体 | `30_` ~ `70_` | 12 | `30_上下文体`、`32_比对假设体`、`34_接驳配置体`、`36_发布运维体`、`44_汇报体`、`45_证据裁决体`、`46_验证体`、`47_文档体`、`48_安全体`、`49_架构规划体`、`69_编码体`、`70_审核体` |

角色系统经过一轮精简合并：原 1 + 11 连结体 + 21 子个体收敛为 1 + 5 + 12，被合并角色的全部职能由合并后的角色继承（职能映射见 [`src/prompt/agents/README_组合指南.md`](src/prompt/agents/README_组合指南.md)）。

> 说明：编号用于维持稳定索引与分发一致性，不连续的编号（30/32/34/36/44/45/46/47/48/49/69/70）是精简后保留的历史索引。

子个体的定位不是"独立人格"，而是按职能复用的功能单元，可以同时出现在多个连结体的常用挂载清单中。
