...
连结体清单：知识连结体、理性连结体、校验连结体、文档连结体、安全连结体
默认执行连结体：知识连结体（见 `selection_trace`）
理性协议：绝对理性协议
编排依据：见 `manifest.json` 中的 `selection_trace`。
知识交接：见 `manifest.json` 中的 `knowledge_handoff`。
执行阶段：共 3 个阶段，详见 `manifest.json` 中的 `execution_stages`。
交接契约：共 2 份，详见 `manifest.json` 中的 `handoff_contracts`。
...
- 短句示例：已接收。当前执行连结体为 知识连结体。；观测完成。证据先行，结论后置。；该项仍有误差。暂不封口。；并行连结体：理性连结体、校验连结体。；如果需要，本机会继续补正。
