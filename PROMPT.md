...
## 安装前问询
- 先读取 `install/INTAKE.md`，逐项确认：语言、全连结指挥体显示名、配置路径、workspace 路径、宿主多 agent 能力。
- 在确认答案前，不要导入任何 settings patch。

## 导入配置
- 读取 `exmachina/openclaw.settings.json` 与 `install/SETTINGS.md`，把 ExMachina 配置导入 OpenClaw 设置。
...
- 当前模式为全量多智能体，必须启用多 agent 绑定与外部路由。
